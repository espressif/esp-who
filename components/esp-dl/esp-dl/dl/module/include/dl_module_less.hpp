...
public:
    ...
    void forward(std::vector<dl::TensorBase *> &tensors, runtime_mode_t mode)
    {
        ...
        // DL_LOG_LAYER_LATENCY_END(this->name, "Less2D");
    }

    void forward_args(void *args)
    {
        if (quant_type == QUANT_TYPE_SYMM_8BIT) {
            base::elemwise_less((base::elemwiseArgsType<int8_t> *)args);
        } else if (quant_type == QUANT_TYPE_SYMM_16BIT) {
            base::elemwise_less((base::elemwiseArgsType<int16_t> *)args);
        }
    }
    ...
    /**
     * @brief deserialize Less module instance by node serialization information
     */
    static Module *deserialize(fbs::FbsModel *fbs_model, std::string node_name)
    {
        ...
        if (quant_type == QUANT_TYPE_SYMM_8BIT || quant_type == QUANT_TYPE_SYMM_16BIT) {
            op = new Less(node_name.c_str(), MODULE_INPLACE_CHANGED_BUFFER, quant_type);
        }
        return op;
    }

    void print() { ESP_LOGI("Less", "quant_type: %s.", quant_type_to_string(quant_type)); }
};
