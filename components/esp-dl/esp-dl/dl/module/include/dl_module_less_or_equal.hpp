...
public:
     ...
     */
    LessOrEqual(const char *name = NULL,
    ...
    /**
     * @brief deserialize LessOrEqual module instance by node serialization information
     */
    static Module *deserialize(fbs::FbsModel *fbs_model, std::string node_name)
    {
        ...
        if (quant_type == QUANT_TYPE_SYMM_8BIT || quant_type == QUANT_TYPE_SYMM_16BIT) {
            op = new LessOrEqual(node_name.c_str(), MODULE_INPLACE_CHANGED_BUFFER, quant_type);
        }
        return op;
    }

    void print() { ESP_LOGI("LessOrEqual", "quant_type: %s.", quant_type_to_string(quant_type)); }
};
