...
public:
    ...
    {
        if (quant_type == QUANT_TYPE_SYMM_8BIT) {
            base::elemwise_max((base::elemwiseArgsType<int8_t> *)args);
        } else if (quant_type == QUANT_TYPE_SYMM_16BIT) {
            base::elemwise_max((base::elemwiseArgsType<int16_t> *)args);
        }
    }
    ...
    void forward_template(std::vector<TensorBase *> &tensors, runtime_mode_t mode)
    {
        ...
        if (task_size == 1) { // single task
            ...
            ESP_LOGE("Max2D", "Only support task size is 1 or 2, currently task size is %d", task_size);
        }
    }

    /**
     * @brief deserialize Max module instance by node serialization information
     */
    static Module *deserialize(fbs::FbsModel *fbs_model, std::string node_name)
    {
        ...
        if (quant_type == QUANT_TYPE_SYMM_8BIT || quant_type == QUANT_TYPE_SYMM_16BIT) {
            op = new Max(node_name.c_str(), MODULE_INPLACE_CHANGED_BUFFER, quant_type);
        }
        return op;
    }

    void print() { ESP_LOGI("Max", "quant_type: %s.", quant_type_to_string(quant_type)); }
};
