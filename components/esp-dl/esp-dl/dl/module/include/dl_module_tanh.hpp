 ...
 * NOTE:
 *
 * @tparam feature_t supports float32, int16_t and int8_t,
 *         - float32 : stands for operation in float32
...
public:
    ...
    void forward(std::vector<dl::TensorBase *> &tensors, runtime_mode_t mode)
    {
        ...
        if (quant_type == QUANT_TYPE_SYMM_8BIT) {
            ...
            for (size_t i = 0; i < input->size; i++) {
                float temp = math::tanh((float)input_ptr[i] * input_scale);
    ...
    /**
     * @brief deserialize Tanh module instance by node serialization information
     */
    ...
    void print() { ESP_LOGI("Tanh", "quant_type: %s.", quant_type_to_string(quant_type)); }
};
} // namespace module
} // namespace dl
