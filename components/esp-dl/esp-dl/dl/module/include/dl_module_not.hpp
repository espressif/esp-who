...
public:
    /**
     * @brief Construct a new Not2D object.
     *
     * @param name            name of module
     * @param inplace         inplace type.
     */
    Not(const char *name = NULL,
    ...
    /**
     * @brief Destroy the Not2D object.
     */
    ~Not() {}
    ...
    void forward(std::vector<dl::TensorBase *> &tensors, runtime_mode_t mode)
    {
        ...
        // DL_LOG_LAYER_LATENCY_END(this->name, "Not2D");
    }

    void forward_args(void *args)
    {
        if (quant_type == QUANT_TYPE_SYMM_8BIT) {
            base::elemwise_not((base::elemwiseArgsType<int8_t> *)args);
        } else if (quant_type == QUANT_TYPE_SYMM_16BIT) {
            base::elemwise_not((base::elemwiseArgsType<int16_t> *)args);
        }
    }
    ...
    void forward_template(std::vector<TensorBase *> &tensors, runtime_mode_t mode)
    {
        ...
        if (task_size == 1) { // single task
            ...
            ESP_LOGE("Not2D", "Only support task size is 1 or 2, currently task size is %d", task_size);
        }
    }

    /**
     * @brief deserialize Not module instance by node serialization information
     */
    static Module *deserialize(fbs::FbsModel *fbs_model, std::string node_name)
    {
        ...
        if (quant_type == QUANT_TYPE_SYMM_8BIT || quant_type == QUANT_TYPE_SYMM_16BIT) {
            op = new Not(node_name.c_str(), MODULE_INPLACE_CHANGED_BUFFER, quant_type);
        }
        return op;
    }

    void print() { ESP_LOGI("Not", "quant_type: %s.", quant_type_to_string(quant_type)); }
};
