...
private:
    std::vector<TensorBase *>
        m_inputs_constant; /*input of Mul. If the input of mul is constant , store its TensorBase pointer; if not, pass
...
public:
    ...
    static Module *deserialize(fbs::FbsModel *fbs_model, std::string node_name)
    {
        ...
        if (quant_type == QUANT_TYPE_SYMM_8BIT || quant_type == QUANT_TYPE_SYMM_16BIT) {
            ...
            op = new Mul(node_name.c_str(), MODULE_NON_INPLACE, quant_type, {input0_constant, input1_constant});
        }
        return op;
    }
