...
void LCD::init_lvgl_dscs()
{
    ...
    m_label_dsc.color = lv_palette_main(LV_PALETTE_RED);
    m_label_dsc.font = &lv_font_montserrat_24;

    lv_draw_rect_dsc_init(&m_rect_dsc);
    m_rect_dsc.bg_opa = LV_OPA_TRANSP;
    m_rect_dsc.border_width = 2;
    m_rect_dsc.border_color = lv_palette_main(LV_PALETTE_RED);

    lv_draw_arc_dsc_init(&m_arc_dsc);
    m_arc_dsc.color = lv_palette_main(LV_PALETTE_RED);
    m_arc_dsc.width = 5;
    m_arc_dsc.radius = 5;
...
void LCD::draw_rectangle(const std::vector<int> &box, const char *label)
{
    ...
    if (label) {
        int32_t h = lv_font_montserrat_24.line_height;
...
void WhoLCD::display_result(who::cam::cam_fb_t *fb)
{
    ...
    for (int type = 0; type < RESULT_TYPE_MAX + 1; type++) {
        ...
        auto it = std::find_if(m_results[type].rbegin(), m_results[type].rend(), [fb](const result_t &result) -> bool {
