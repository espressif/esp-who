...
```
E (1137) ESP_MAC_WIFI_STA ADDRESS: 7cdfa1e0ab78

I (1137) gpio: GPIO[0]| InputEn: 1| OutputEn: 0| OpenDrain: 0| Pullup: 1| Pulldown: 0| Intr:3 
I (2237) ESP32-S3-EYE-V2.1: --------------- Enter Test Mode ---------------

ESP32-S3-EYE V2.1 Firmware EN V0.1.3
I (2236) spi_bus: SPI2 bus created
I (2236) spi_bus: SPI2 bus device added, CS=44 Mode=0 Speed=40000000
I (2236) lcd st7789: MADCTL=0
I (2236) LCD: Screen name:ST7789 | width:240 | height:240
W (2756) eFuse: calibration efuse version does not match, set default version: 0
I (2756) i2c_bus: i2c0 bus inited
W (2756) qma7981: ID : E7
I (2756) gpio: GPIO[3]| InputEn: 0| OutputEn: 1| OpenDrain: 1| Pullup: 1| Pulldown: 0| Intr:0 
Initializing SPIFFS
Partition size: total: 3287096, used: 3070232
model_name: hiesp8q8 model_data: /srmodel/hiesp8q8/wn8q8_data
HUFZIP MC Quantized wakeNet8Q8: WakeNet8Q8_v2_hiesp_5_0.57_0.50, mode:2, p:1, (Nov 23 2021 15:19:36)
I (3756) ESP32-S3-EYE: --------------- Test Start ---------------
...
W (1632756) MIC: Please Say "Hi, ESP"
...
3. 人脸识别模式：该模式下LCD会实时显示摄像头拍摄到的画面，如果画面中存在人脸，LCD显示的画面上会框出人脸，并画出人脸的五个特征点。该模式下提供三种功能：录入人脸、识别人脸、删除人脸。
   - 录入人脸： 按下MENU键。或者使用“Hi ESP”唤醒词唤醒，唤醒后摄像头旁的LED会被点亮，随后说出“enter face”。
   - 识别人脸： 按下UP键。或者使用“Hi ESP”唤醒词唤醒，唤醒后摄像头旁的LED会被点亮，随后说出“recognize face”。
   - 删除人脸： 按下PLAY键。或者使用“Hi ESP”唤醒词唤醒，唤醒后摄像头旁的LED会被点亮，随后说出“delete face”。
4. 移动侦测模式： 该模式下LCD会实时显示摄像头拍摄到的画面。若画面中存在物体移动，LCD中左上角会显示蓝色色块。

#### 模式切换
在任何时候都可以使用“Hi ESP”唤醒词唤醒开发板，四种模式分别对应四个命令词。说出相应的命令词，识别成功后随即切换到对应的模式。工作模式与命令词的对应关系如下：
1. 待机模式：“stop working”
2. 实时显示模式：“display only”
3. 人脸识别模式：“face recognition”
4. 移动侦测模式：“motion detection”

#### 语音系统
- 语音唤醒：默认固件工作时，任何时刻都可以使用“Hi ESP”唤醒词唤醒开发板。开发板被唤醒后，摄像头旁的LED会常亮。
