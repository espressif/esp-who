...
static void enc_list_mtf(iconv_t _enc_list[4],iconv_t _enc){
  int i;
  for(i = 0; i < 4; i++)
    if(_enc_list[i] == _enc){
      int j;
                                   ...
                                   zbar_image_t *img)
{
  ...
  mark=(unsigned char *)heap_caps_calloc(nqrdata,sizeof(*mark), MALLOC_CAP_SPIRAM);
  ntext=0;
...
//  latin1_cd=iconv_open("utf-8","GB18030");
  ...
  for(i = 0; i < nqrdata; i++)
    if(!mark[i]){
      ...
      // printf("sa_size:%d, has_kanji: %d\n",sa_size, has_kanji);

      /*Step 2: Convert the entries.*/
      // sa_text=(char *)malloc((sa_ctext+1)*sizeof(*sa_text));
      sa_text=(char *)heap_caps_malloc((sa_ctext+1)*sizeof(*sa_text), MALLOC_CAP_SPIRAM);
      sa_ntext=0;
      ...
      char *bytebuf_text = (char *)heap_caps_malloc((sa_ctext+1)*sizeof(*sa_text), MALLOC_CAP_SPIRAM);
      ...
      for(j = 0; j < sa_size && !err; j++, sym = &(*sym)->next) {
        *sym = _zbar_image_scanner_alloc_sym(iscn, ZBAR_QRCODE, 0);
        ...
        for(k = 0; k < qrdataj->nentries && !err; k++){
          ...
          // printf("k:%d, bytebuf_text:%s, bytebuf_ntext:%d\n",k,bytebuf_text,bytebuf_ntext);
          ...
          if(bytebuf_ntext > 0){
            ...
            if(entry == NULL || (entry->mode != (qrdataj->entries+k-1)->mode) )
            { //|| (entry->mode != QR_MODE_BYTE && entry->mode != QR_MODE_KANJI)
              // printf("eci: %d\n",eci);
              in=bytebuf_text;
              ...
              if(eci<0){
                ...
                else if(inleft>=3&& in[0]==(char)0xEF&&in[1]==(char)0xBB&&in[2]==(char)0xBF){
                  in+=3;
                  inleft-=3;
              ...
              /*We were actually given a character set; use it.
                ...
                /* for checking eci mode, printf input buffer(in) in hex(%x), 
                check the hex value in the encoding table to see if it's supported, 
                for example 0xc2eb ---gb2312--> 码 
                if the hex value cannot be found in the table then need to look for a more detailed table*/
                // printf("in:%s, inleft:%d: ", in, inleft);
                ...
                err=eci_cd==(iconv_t)-1||
                 ...
                 // printf("in:%s, out:%s\n", in, out);
                 // printf("err:%d, %d\n",eci_cd==(iconv_t)-1,iconv(eci_cd,&in,&inleft,&out,&outleft)==(size_t)-1);
          ...
          switch(entry->mode){
            case QR_MODE_NUM:{
              ...
              // printf("sa_text:%s, sa_ntext:%d\n",sa_text, sa_ntext);
            }
            break;
            case QR_MODE_ALNUM:{
              // printf("QR_MODE_ALNUM\n");
              char *p;
            ...
            case QR_MODE_ECI:{
              ...
              else if(cur_eci==QR_ECI_BIG5)enc="BIG5";
              else if(cur_eci==QR_ECI_GB2312)enc="GB2312";
              /*Don't know what this ECI code specifies, but not an encoding that
                 we recognize.*/
              else continue;
              eci=cur_eci;
              eci_cd=iconv_open("UTF-8",enc);
              // printf("enc:%s, sa_text:%s, sa_ntext:%d\n",enc, sa_text, sa_ntext);
            }break;
      ...
      // copy bytebuf_text to sa_text ???
      memcpy(sa_text+sa_ntext, bytebuf_text, bytebuf_ntext *sizeof(*bytebuf_text));
      sa_ntext += bytebuf_ntext;
      // printf("sa_text:%s\n",sa_text);
      ...
      if(!err){
        ...
        if(sa_ctext+1>sa_ntext){
          ...
          sa_text=(char *)heap_caps_realloc(sa_text,sa_ntext*sizeof(*sa_text), MALLOC_CAP_SPIRAM);
        }
