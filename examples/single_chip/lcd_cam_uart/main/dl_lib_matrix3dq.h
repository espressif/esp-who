...
#if CONFIG_IDF_TARGET_ESP32S3BETA
#include "esp32s3beta/rom/cache.h"
#define DL_S3 1
#define DEBUG_CACHE_PRELOAD 1
#else
#define DL_S3 0
#endif
extern SemaphoreHandle_t g_op_done_sem[portNUM_PROCESSORS];

void dl_matrix3dq_op_init(void);
...
static inline dl_matrix3dq_t *dl_matrix3dq_alloc(int n, int w, int h, int c, int e)
{
    ...
    if (NULL == items)
    {
        printf("matrix3dq item alloc failed. n, w, h, c: %d %d %d %d\n", n, w, h, c);
        dl_lib_free(r);
        return NULL;
    }
...
static inline bool dl_matrix3dq_tie_verify(dl_matrix3dq_t *f, int mode)
{
    if (mode == DL_TIE_IMPL)
    {
        if (f->h == 1 && f->w == 1)
        {
            if (f->n % 8 == 0 && f->c >= 4 && f->c % 2 == 0)
            {
                return true;
            }
        }
...
static inline bool dl_matrix3dq_xtensa_verify(dl_matrix3dq_t *f, int mode)
{
    if (mode >= DL_XTENSA_IMPL && f->c >= 4)
        return true;
 ...
 */
dl_matrix3dq_t *dl_matrix3dq_concat(dl_matrix3dq_t *in_1, dl_matrix3dq_t *in_2, char *name);
 ...
 * @param relu      RELU activation after adding bias, 0 for no, 1 for yes
 * @param mode      Implementation mode, DL_TIE_IMPL, DL_XTENSA_IMPL, DL_C_IMPL
...
/**
 * @brief Do 1x1 convolution with a quantized matrix, activation is PRELU
 *
 * @param out       Preallocated quantized matrix, size (1, w, h, n)
 * @param in        Input matrix, size (1, w, h, c)
 * @param filter    1x1 filter, size (n, 1, 1, c)
 * @param bias      Bias, size (1, 1, 1, n)
 * @param prelu     PRelu, size (1, 1, 1, n)
 * @param mode      Implementation mode, DL_TIE_IMPL, DL_XTENSA_IMPL, DL_C_IMPL
 * @param name      Name of layer
 */
...
/**
 * @brief Do 1x1 convolution with input being 8-bit fixed point matrix, usually for first layer, where input is image
 *
 * @param out       Preallocated quantized matrix, size (1, w, h, n)
 * @param in        Input matrix, size (1, w, h, c)
 * @param filter    1x1 filter, size (n, 1, 1, c)
...
/**
 * @brief 
 * if(out == NULL) the result will be updated in x1, else in out
 * @param out 
 * @param x1 
 * @param x2 
 * @param out_exponent 
 */
void dl_matrix3dqq_element_product(dl_matrix3dq_t *out, dl_matrix3dq_t *x1, dl_matrix3dq_t *x2, int out_exponent);
...
dl_matrix3dq_t *dl_matrix3dqq_mobilefaceblock_preload2(dl_matrix3dq_t *in,
                                                       dl_matrix3dq_t *pw,
                                                       dl_matrix3dq_t *pw_bias,
                                                       dl_matrix3dq_t *dw,
                                                       dl_matrix3dq_t *dw_bias,
                                                       ...
                                                       int mode,
                                                       int shortcut,
                                                       dl_matrix3dq_preload_t *preload);
dl_matrix3dq_t *dl_matrix3dqq_mobilefaceblock_preload(dl_matrix3dq_t *in,
                                                      dl_matrix3dq_t *pw,
                                                      dl_matrix3dq_t *pw_bias,
                                                      dl_matrix3dq_t *dw,
                                                      dl_matrix3dq_t *dw_bias,
