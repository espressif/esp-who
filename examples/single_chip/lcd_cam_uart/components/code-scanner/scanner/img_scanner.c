                               ...
                               int datalen)
{
    ...
    if(sym) {
        ...
        sym = heap_caps_calloc(1, sizeof(zbar_symbol_t),MALLOC_CAP_SPIRAM);
        STAT(sym_new);
    }
    ...
    if(datalen > 0) {
        ...
        if(sym->data_alloc < datalen) {
            ...
            sym->data = heap_caps_malloc(datalen,MALLOC_CAP_SPIRAM);
        }
    }
    else {
        if(sym->data)
...
static inline void qr_handler (zbar_image_scanner_t *iscn)
{
    ...
    // the value here is the value in the subpixel mode, which is the [orginal pixel pos << QR_FINDER_SUBPREC(2)]
    ...
    if(iscn->du < 0) { // if the direction is from end to start
        u -= line->len;
        int tmp = line->boffs;
    ...
    // printf("line->len: %d, line->boffs: %d, line->eoffs: %d\n", line->len, line->boffs, line->eoffs);
...
zbar_image_scanner_t *zbar_image_scanner_create ()
{
    ...
    zbar_image_scanner_t *iscn = heap_caps_calloc(1, sizeof(zbar_image_scanner_t),MALLOC_CAP_SPIRAM);
    if(!iscn)
        return(NULL);
...
//                      zbar_image_t *img)
{
    ...
    if(density > 0) {
        ...
        while(y < cy1) {
            ...
            while(x < cx1) {
                uint8_t d = *p;
                movedelta(1, 0);
                edge_typ = zbar_scan_y(scn, d);
                if (edge_typ == ZBAR_PARTIAL)
                    partial_count ++;
                else if (edge_typ == ZBAR_QRCODE)
                    qr_count ++;
                else if(edge_typ == ZBAR_CODE93 || edge_typ == ZBAR_CODE128)
                    barc_count ++;
            }
            ASSERT_POS;
            quiet_border(iscn);
    ...
    // printf("%d,%d,%d\n", partial_count,qr_count,barc_count);
...
#if ENABLE_QRCODE == 1
    int nhclusters;
    int *range = qr_finder_find_vrange(iscn->qr, iscn, img, &nhclusters);
    // printf("nhclusters:%d\n", nhclusters);

    if(nhclusters > 0){

        // int *flag = (int *)malloc(nhclusters*sizeof(flag));
        // int *nvline = (int *)malloc(nhclusters*sizeof(nvline));
        int *flag = (int *)heap_caps_malloc(nhclusters*sizeof(flag), MALLOC_CAP_SPIRAM);
        int *nvline = (int *)heap_caps_malloc(nhclusters*sizeof(nvline), MALLOC_CAP_SPIRAM);
        int pre_nvline = 0, nvcluster = 0;

        for(int i=0; i<nhclusters; i++){
       	    // printf("%d, %d, %d, %d\n", range[4*i], range[4*i+1],range[4*i+2],range[4*i+3]);

        	density = CFG(iscn, ZBAR_CFG_X_DENSITY);
        	if(density > 0) {
        		int x = range[4*i], y = range[4*i+1];
        		const uint8_t *p = data + (y * w + x);

        		cx1 = range[4*i] + range[4*i+2] > w ? w: range[4*i] + range[4*i+2];
        		cy1 = range[4*i+1] + range[4*i+3] > h ? h : range[4*i+1] + range[4*i+3];
        	...
        	int line = _qr_get_nvline(iscn->qr);
        	if(line - pre_nvline <= 2){
        		flag[i] = 0;
        		nvline[i] = line - pre_nvline;
        	}
        	else{
        		flag[i] = 1;
        		nvline[i] = line - pre_nvline;
        ...
        if (nvcluster > 0){
            // printf("nvcluster:%d\n", nvcluster);
            qr_finder_get_vcluster(iscn->qr, flag, nvline, pre_nvline, nvcluster);
