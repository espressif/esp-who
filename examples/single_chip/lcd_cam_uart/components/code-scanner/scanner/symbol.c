...
unsigned zbar_base64_encode (char *dst,
                        const char *src,
                        unsigned srclen) // rename it because the conflict with wpa_supplicant/src/wpa2/utils/base64.c:31
{
                       ...
                       unsigned *len)
{
    ...
    if(!*buf || (*len < maxlen)) {
        if(*buf)
            free(*buf);
        // *buf = malloc(maxlen);
        *buf = heap_caps_malloc(maxlen, MALLOC_CAP_SPIRAM);
        /* FIXME check OOM */
        *len = maxlen;
    }
...
zbar_symbol_set_t *_zbar_symbol_set_create ()
{
    ...
    zbar_symbol_set_t *syms = heap_caps_calloc(1, sizeof(*syms), MALLOC_CAP_SPIRAM);
