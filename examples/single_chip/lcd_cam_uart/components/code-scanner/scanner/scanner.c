...
zbar_scanner_t *zbar_scanner_create (zbar_decoder_t *dcode)
{
    ...
    zbar_scanner_t *scn = heap_caps_malloc(sizeof(zbar_scanner_t), MALLOC_CAP_SPIRAM);
    scn->decoder = dcode;
...
static inline zbar_symbol_type_t process_edge (zbar_scanner_t *scn,
                                               int y1)
{
    //modified to detect quiet zone(border width != 0)
   if(!scn->y1_sign)
    ...
    // printf("process_edge, sgn=%d cur=%d.%d w=%d (%s)\n",
...
inline zbar_symbol_type_t zbar_scanner_flush (zbar_scanner_t *scn)
{
    ...
    if(scn->cur_edge != x || scn->y1_sign > 0) { //ori: scn->y1_sign > 0
                                                // modified to detect quiet zone(allow end border to be processed)
        // dprintf(1, "flush0:");
                                ...
                                int y)
{
    ...
    //dprintf(1, "scan: x=%d y=%d y0=%d y1=%d y2=%d",x, y, y0_1, y1_1, y2_1);
    edge = ZBAR_NONE;
    ...
    {
        // printf("x=%d y=%d y0=%d y1_1=%d y2_1=%d y2_2=%d\n",x, y, y0_1, y1_1, y2_1, y2_2);

        /* check for 1st sign change */
        char y1_rev = (scn->y1_sign > 0) ? y1_1 < 0 : y1_1 > 0;
        // printf("scn->y1_sign: %d, y1_rev: %d\n", scn->y1_sign, y1_rev);
        ...
        if(y1_rev || (abs(scn->y1_sign) < abs(y1_1))) {
            ...
            // printf("y1_thresh=%d, d: %d, cur_edge: %d\n", scn->y1_thresh, d, scn->cur_edge);
        }
    }
    else{
        // dbprintf(1, "\n");
