...
zbar_image_t *zbar_image_create ()
{
    ...
    zbar_image_t *img = heap_caps_calloc(1, sizeof(zbar_image_t),MALLOC_CAP_SPIRAM);
    _zbar_refcnt_init();
                      ...
                      const char *filebase)
{
    ...
    char *filename = heap_caps_malloc(len, MALLOC_CAP_SPIRAM);
    int n = 0, rc = 0;
    FILE *f;
    zimg_hdr_t hdr;
                          ...
                          const char *filename)
{
    ...
    rows = heap_caps_malloc(img->height * sizeof(*rows), MALLOC_CAP_SPIRAM);
    if(!rows)
        goto done;
