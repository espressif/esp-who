...
int qr_ilog(unsigned _v){  // 32 - CLZ
#if defined(QR_CLZ)    // CLZ: counts number of zeros before the first occurence of one(set bit)
/*Note that __builtin_clz is not defined when _x==0, according to the gcc
  ...
  int ret;
  int m;
  // if high four-0x bit is 0000, m=0, means nothing; 
  // if high four-0x bit is not 0000, m=16, remove the lowest 16 bit, then check the high 16 bit 
  m=!!(_v&0xFFFF0000)<<4; 
  _v>>=m;
  ret=m;
  m=!!(_v&0xFF00)<<3;
