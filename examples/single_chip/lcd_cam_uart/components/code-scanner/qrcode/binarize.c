...
//        int *pixelCount = (int*)malloc(num * sizeof(*pixelCount));
//        for(int i=0; i<num; i++)
//            pixelCount[i] = 0;
//
//        // get five rows and calculate the histogram for 3/5 width of these rows
//        for (int y = 1; y < 5; y++) {
//            int row = _height * y / 5;
//            int right = (_width << 2) / 5;
//            for (int x = _width / 5; x < right; x++) {
//                pixelCount[(int)_img[row * _width + x]] ++;
//            }
//        }
//
...
//
//        for (int i = 0; i < num; i++) {
//            if (pixelCount[i] > firstPeakSize) {
//              firstPeak = i;
//              firstPeakSize = pixelCount[i];
//            }
...
//            int score = pixelCount[i] * distanceToBiggest * distanceToBiggest;
...
//    }
   ...
   if(_width>0 && _height>0){
     ...
     int            y;
     // mask = (unsigned char *)malloc(_width*_height*sizeof(*mask));
     mask = (unsigned char *)heap_caps_malloc(_width*_height*sizeof(*mask), MALLOC_CAP_SPIRAM);
     ...
     windw = win_w;
     windh = win_h;
#if defined(QR_BINARY_WINDOW_SIZE)
     printf("windw: %d, windh: %d\n", windw, windh);
#endif

     // col_sums = (unsigned *)malloc(_width*sizeof(*col_sums));
     col_sums = (unsigned *)heap_caps_malloc(_width*sizeof(*col_sums), MALLOC_CAP_SPIRAM);
     /*Initialize sums down each column.*/
     for(x = 0; x < _width; x++){
       g = _img[x];
       // col_sums[x] = (g << (logwindh - 1)) + g;
       col_sums[x] = (g * (windh >> 1)) + g;
     }
     for(y = 1 ; y < (windh >> 1); y++){
       y1offs = QR_MINI(y,_height-1) * _width;
       for(x = 0; x < _width; x++){
     ...
     for(y = 0; y < _height; y++){
       ...
       // m = (col_sums[0] << (logwindw - 1))+col_sums[0];
       m = (col_sums[0] * (windw>>1))+col_sums[0];
       for(x = 1;x < (windw>>1); x++){
         x1 = QR_MINI(x,_width-1);
         m += col_sums[x1];
       }
       for(x=0; x < _width; x++){
         ...
         mask[y*_width+x] = -( (g + 3) * (windw*windh) <m)&0xFF;
...
#if defined(QR_DEBUG_BINARY)
   for(int j = 0; j < _height; j ++){
	   	  for(int i = 0; i < _width; i ++){ // +=2 for screen output

    		  printf("%d,",mask[j*_width + i]);
    	  }
    	  printf(";\n");
   }
 ...
 //  printf("\n\binarize\n");
 //  for(int i = 0; i < _width; i += 3){ // +=2 for screen output
 //	  for(int j = 0; j < _height; j += 3){
 //		  if(mask[i*_height+j] == 0)
 //			  printf("* ");
 //		  else if(mask[i*_height+j] == 255)
 //			  printf("  ");
 //		  else
 //			  printf("-");
 //	  }
