...
#define QR_SIGNI(_x)        (((_x)>0)-((_x)<0))    // when negative, = -1; when positive, = 1;
#define QR_SIGNMASK(_x)     (-((_x)<0))           // when negative, = -1; when positive, = 0;
/*Unlike copysign(), simply inverts the sign of _a if _b is negative.*/
#define QR_FLIPSIGNI(_a,_b) ( ( (_a)+QR_SIGNMASK(_b) )^QR_SIGNMASK(_b))     // when b is negative, a = -a;
#define QR_COPYSIGNI(_a,_b) QR_FLIPSIGNI(abs(_a),_b)
/*Divides a signed integer by a positive value with exact rounding.*/
#define QR_DIVROUND(_x,_y)  (((_x)+QR_FLIPSIGNI(_y>>1,_x))/(_y))        // x>0, =x/y+0.5; x<0, =x/y-0.5;
#define QR_CLAMPI(_a,_b,_c) (QR_MAXI(_a,QR_MINI(_b,_c)))
#define QR_CLAMP255(_x)     ((unsigned char)((((_x)<0)-1)&((_x)|-((_x)>255))))        // x>255 =1; 0<x<255, =x; x<0, =0;
