...
#define CONFIG_SCCB_HARDWARE_I2C (0)

#if !CONFIG_SCCB_HARDWARE_I2C
...
#else
        esp_err_t ret = esp_mfi_i2c_probe(( slave_addr << 1 ));
#endif
        if( ret == ESP_OK) {
...
uint8_t SCCB_Read(uint8_t slv_addr, uint8_t reg)
{
    ...
    i2c_cmd_link_delete(cmd);
#else
    ret = esp_mfi_i2c_read(( slv_addr << 1 ), reg, &data, 1);
#endif
    if(ret != ESP_OK) {
...
uint8_t SCCB_Write(uint8_t slv_addr, uint8_t reg, uint8_t data)
{
    ...
    i2c_cmd_link_delete(cmd);
#else
    ret = esp_mfi_i2c_write(( slv_addr << 1 ), reg, &data, 1);
#endif
    if(ret != ESP_OK) {
...
uint8_t SCCB_Read16(uint8_t slv_addr, uint16_t reg)
{
    ...
    i2c_cmd_link_delete(cmd);
#else
    ret = esp_mfi_i2c_read16(( slv_addr << 1 ), reg, &data, 1);
#endif
    if(ret != ESP_OK) {
...
uint8_t SCCB_Write16(uint8_t slv_addr, uint16_t reg, uint8_t data)
{
    ...
    i2c_cmd_link_delete(cmd);
#else
    ret = esp_mfi_i2c_write16(( slv_addr << 1 ), reg, &data, 1);
#endif
    if(ret != ESP_OK) {
