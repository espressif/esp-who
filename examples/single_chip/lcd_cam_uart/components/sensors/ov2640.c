...
#define SCCB_ID   			0X30  			//OV2640的ID

#define delay_ms(a) vTaskDelay(a / portTICK_RATE_MS);

//初始化OV2640，XCLK输入12MHz时钟
...
uint8_t OV2640_Init(uint8_t mode, uint8_t fre_double_en)
{
    ...
    SCCB_Write(SCCB_ID, OV2640_DSP_RA_DLMT, 0x01);	//操作sensor寄存器
    SCCB_Write(SCCB_ID, OV2640_SENSOR_COM7, 0x80);	//软复位OV2640
    delay_ms(50);
    reg = SCCB_Read(SCCB_ID, OV2640_SENSOR_MIDH);	//读取厂家ID 高八位
    reg <<= 8;
    reg |= SCCB_Read(SCCB_ID, OV2640_SENSOR_MIDL);	//读取厂家ID 低八位
    ...
    reg = SCCB_Read(SCCB_ID, OV2640_SENSOR_PIDH);	//读取厂家ID 高八位
    reg <<= 8;
    reg |= SCCB_Read(SCCB_ID, OV2640_SENSOR_PIDL);	//读取厂家ID 低八位
...
void OV2640_Window_Set(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
    ...
    SCCB_Write(SCCB_ID, 0XFF, 0X01);
    temp = SCCB_Read(SCCB_ID, 0X03);				//读取Vref之前的值
    temp &= 0XF0;
    temp |= ((endy & 0X03) << 2) | (sy & 0X03);
    SCCB_Write(SCCB_ID, 0X03, temp);				//设置Vref的start和end的最低2位
    SCCB_Write(SCCB_ID, 0X19, sy >> 2);			//设置Vref的start高8位
    SCCB_Write(SCCB_ID, 0X1A, endy >> 2);			//设置Vref的end的高8位

    temp = SCCB_Read(SCCB_ID, 0X32);				//读取Href之前的值
    temp &= 0XC0;
    temp |= ((endx & 0X07) << 3) | (sx & 0X07);
    SCCB_Write(SCCB_ID, 0X32, temp);				//设置Href的start和end的最低3位
    SCCB_Write(SCCB_ID, 0X17, sx >> 3);			//设置Href的start高8位
    SCCB_Write(SCCB_ID, 0X18, endx >> 3);			//设置Href的end的高8位
}
//设置图像输出大小
...
uint8_t OV2640_OutSize_Set(uint16_t width, uint16_t height)
{
    ...
    SCCB_Write(SCCB_ID, 0X5A, outw & 0XFF);		//设置OUTW的低八位
    SCCB_Write(SCCB_ID, 0X5B, outh & 0XFF);		//设置OUTH的低八位
    temp = (outw >> 8) & 0X03;
    temp |= (outh >> 6) & 0X04;
    SCCB_Write(SCCB_ID, 0X5C, temp);				//设置OUTH/OUTW的高位
...
uint8_t OV2640_ImageWin_Set(uint16_t offx, uint16_t offy, uint16_t width, uint16_t height)
{
    ...
    SCCB_Write(SCCB_ID, 0XFF, 0X00);
    SCCB_Write(SCCB_ID, 0XE0, 0X04);
    SCCB_Write(SCCB_ID, 0X51, hsize & 0XFF);		//设置H_SIZE的低八位
    SCCB_Write(SCCB_ID, 0X52, vsize & 0XFF);		//设置V_SIZE的低八位
    SCCB_Write(SCCB_ID, 0X53, offx & 0XFF);		//设置offx的低八位
    SCCB_Write(SCCB_ID, 0X54, offy & 0XFF);		//设置offy的低八位
    temp = (vsize >> 1) & 0X80;
    temp |= (offy >> 4) & 0X70;
    temp |= (hsize >> 5) & 0X08;
    temp |= (offx >> 8) & 0X07;
    SCCB_Write(SCCB_ID, 0X55, temp);				//设置H_SIZE/V_SIZE/OFFX,OFFY的高位
    SCCB_Write(SCCB_ID, 0X57, (hsize >> 2) & 0X80);	//设置H_SIZE/V_SIZE/OFFX,OFFY的高位
    SCCB_Write(SCCB_ID, 0XE0, 0X00);
    return 0;
}
...
uint8_t OV2640_ImageSize_Set(uint16_t width, uint16_t height)
{
    ...
    SCCB_Write(SCCB_ID, 0XFF, 0X00);
    SCCB_Write(SCCB_ID, 0XE0, 0X04);
    SCCB_Write(SCCB_ID, 0XC0, (width) >> 3 & 0XFF);		//设置HSIZE的10:3位
    SCCB_Write(SCCB_ID, 0XC1, (height) >> 3 & 0XFF);		//设置VSIZE的10:3位
    temp = (width & 0X07) << 3;
    temp |= height & 0X07;
    temp |= (width >> 4) & 0X80;
