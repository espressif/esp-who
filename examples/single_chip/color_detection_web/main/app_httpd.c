...
static int yellow_thresh[6] = {18,36,165,255,50,255};
static int green_thresh[6] = {36,63,50,255,50,255};
static int blue_thresh[6] = {70,100,50,255,89,255};
static int red_thresh[6] = {0,17,160,255,50,255};
int color_thresh[6] = {80,220,80,105,95,120};

static float color_sampling_rect[4] = {0.4,0.4,0.2,0.2};

#define AREATHRESH 256
...
static int8_t sampling_enabled = 0;
...
static void draw_color_boxes(dl_matrix3du_t *image_matrix, componentsStats *stats, int areaThresh, uint32_t color)
{
    if (stats == NULL)
    {
        return NULL;
    ...
    {
        // rectangle box
        if (stats->area[i] > areaThresh)
        {
            x = (int)stats->xmin[i];
            y = (int)stats->ymin[i];
            w = (int)stats->xmax[i] - x + 1;
            h = (int)stats->ymax[i] - y + 1;
            printf("gggggg, x:%d, y:%d, w:%d, h:%d, area:%d\n", x, y, w, h, stats->area[i]);
            fb_gfx_drawFastHLine(&fb, x, y, w, color);
            fb_gfx_drawFastHLine(&fb, x, y + h - 1, w, color);
            fb_gfx_drawFastVLine(&fb, x, y, h, color);
            fb_gfx_drawFastVLine(&fb, x + w - 1, y, h, color);
            fb_gfx_fillRect(&fb, stats->xcent[i], stats->ycent[i], 3, 3, centColor);
        }
    }
}

static void draw_boxes(dl_matrix3du_t *image_matrix, float xRatio, float yRatio, float wRatio, float hRatio)
{
    ...
    int x = (int)((image_matrix->w) * xRatio);
    int y = (int)((image_matrix->h) * yRatio);
    int w = (int)((image_matrix->w) * wRatio);
    int h = (int)((image_matrix->h) * hRatio);

    // ets_printf("x:%d,y:%d,w:%d,h:%d\n",x,y,w,h);
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    bool s;
    bool detected = false;
    if (!detection_enabled || fb->width > 800)
    {
        size_t fb_len = 0;
    ...
    componentsStats *stats_yellow = color_detection_forward(image_matrix, yellow_thresh, 1);
    if (stats_yellow)
    {
        detected = true;
        draw_color_boxes(image_matrix, stats_yellow, AREATHRESH, COLOR_YELLOW);
        componentsStats_free(stats_yellow);
    }
    ...
    int64_t fr_end = esp_timer_get_time();
    ESP_LOGI(TAG, "FACE: %uB %ums %s", (uint32_t)(jchunk.len), (uint32_t)((fr_end - fr_start) / 1000), detected ? "DETECTED " : "");
    return res;
}
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        componentsStats *stats_color = NULL;
                        if (detection_enabled && (!sampling_enabled))
                        {
                            stats_color = color_detection_forward(image_matrix, color_thresh, 1);
                        }
                        fr_face = esp_timer_get_time();
                        fr_recognize = fr_face;
                        if (stats_color || fb->format != PIXFORMAT_JPEG || sampling_enabled)
                        {
                            if (stats_color)
                            {
                                detected = true;
                                draw_color_boxes(image_matrix, stats_color, AREATHRESH, COLOR_GREEN);
                                componentsStats_free(stats_color);
                                fr_recognize = esp_timer_get_time();
                            }
                            if (sampling_enabled){
                                draw_boxes(image_matrix,color_sampling_rect[0],color_sampling_rect[1],color_sampling_rect[2],color_sampling_rect[3]);
                                if(is_enrolling){
                                    get_color_detection_thresh(image_matrix, color_sampling_rect, color_thresh);
                                    is_enrolling = 0;
                                }
                            }
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    *p++ = '{';
    ...
    p += sprintf(p, "\"face_recognize\":%u", sampling_enabled);
    *p++ = '}';
    *p++ = 0;
