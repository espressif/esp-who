...
#define HAND_COLOR_WHITE 0x00FFFFFF
#define HAND_COLOR_BLACK 0x00000000
#define HAND_COLOR_RED 0x000000FF
#define HAND_COLOR_GREEN 0x0000FF00
#define HAND_COLOR_BLUE 0x00FF0000
#define HAND_COLOR_YELLOW (HAND_COLOR_RED | HAND_COLOR_GREEN)
#define HAND_COLOR_CYAN (HAND_COLOR_BLUE | HAND_COLOR_GREEN)
#define HAND_COLOR_PURPLE (HAND_COLOR_BLUE | HAND_COLOR_RED)
#endif
...
static int hp_target_size = HP_TARGET_SIZE;
#endif
...
{
    int x, y, w, h, i;
    uint32_t color = HAND_COLOR_GREEN;
    fb_data_t fb;
    fb.width = image_matrix->w;
    fb.height = image_matrix->h;
    fb.data = image_matrix->item;
    ...
    {
        // rectangle box
        x = max(min((int)boxes->box[i].box_p[0], image_matrix->w-1), 0);
        y = max(min((int)boxes->box[i].box_p[1], image_matrix->h-1), 0);
        w = min((int)boxes->box[i].box_p[2] - x + 1, (int)image_matrix->w - x);
        h = min((int)boxes->box[i].box_p[3] - y + 1, (int)image_matrix->h - y);
...
static void draw_landmarks(dl_matrix3du_t *image_matrix, dl_matrix3d_t *landmarks)
{
    uint32_t color = HAND_COLOR_RED;
    int count = 0;
    fb_data_t fb;
    fb.width = image_matrix->w;
    fb.height = image_matrix->h;
    ...
    for (int i = 0; i < landmarks->n; i++)
    {
        // landmark
        int x0, y0, j;
        for (j = 0; j < landmarks->h; j++) {
            x0 = (int)landmarks->item[count];
            y0 = (int)landmarks->item[count+1];
            count += 2;
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    int64_t hp_start = esp_timer_get_time();
    ...
    if (!detection_enabled || fb->width > 400)
    {
        ...
        ESP_LOGI(TAG, "JPG: %uB %ums", (uint32_t)(fb_len), (uint32_t)((hp_end - hp_start) / 1000));
        return res;
#if CONFIG_ESP_HAND_DETECT_ENABLED
    }
    ...
    if (net_boxes)
    {
        ...
        {
            landmarks = handpose_estimation_forward(image_matrix, net_boxes, hp_target_size);
            draw_landmarks(image_matrix, landmarks);
            dl_matrix3d_free(landmarks);
        }
#endif
        draw_boxes(image_matrix, net_boxes);
    ...
    int64_t hp_end = esp_timer_get_time();
    ESP_LOGI(TAG, "HAND: %uB %ums", (uint32_t)(jchunk.len), (uint32_t)((hp_end - hp_start) / 1000));
    return res;
#endif
}
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        hp_ready = esp_timer_get_time();
                        od_box_array_t *net_boxes = NULL;
                        if (detection_enabled)
                        {
                            hd_config_t hd_config = hd_init_config();
                            net_boxes = hand_detection_forward(image_matrix, hd_config);

                        }
                        hp_detect = esp_timer_get_time();
                        hp_pose = hp_detect;
...
#if CONFIG_ESP_HAND_DETECT_ENABLED
        int64_t ready_time = (hp_ready - hp_start) / 1000;
        int64_t detect_time = (hp_detect - hp_ready) / 1000;
        int64_t pose_time = (hp_pose - hp_detect) / 1000;
        int64_t encode_time = (hp_encode - hp_pose) / 1000;
        int64_t process_time = (hp_encode - hp_start) / 1000;
#endif

        int64_t frame_time = hp_end - last_frame;
        last_frame = hp_end;
        frame_time /= 1000;
...
#if CONFIG_ESP_HAND_DETECT_ENABLED
                 ,
                 (uint32_t)ready_time, (uint32_t)detect_time, (uint32_t)pose_time, (uint32_t)encode_time, (uint32_t)process_time,
                 (detected) ? "DETECTED " : ""
#endif
        );
    }
...
#if CONFIG_ESP_HAND_DETECT_ENABLED
    else if (!strcmp(variable, "hand_detect")) {
...
#if CONFIG_ESP_HAND_POSE_ESTIMATION_ENABLED
    else if (!strcmp(variable, "hand_pose")) {
...
#endif
#if CONFIG_ESP_HAND_DETECT_ENABLED
    p += sprintf(p, ",\"hand_detect\":%u", detection_enabled);
#if CONFIG_ESP_HAND_POSE_ESTIMATION_ENABLED
    p += sprintf(p, "\"hand_pose\":%u", recognition_enabled);
#endif
#endif
    *p++ = '}';
    *p++ = 0;
